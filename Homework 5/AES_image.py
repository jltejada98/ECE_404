...
def ctr_aes_image(iv,image_file='image.ppm',out_file='enc_image.ppm',key_file='key.txt'):
    ...
    for image_bv_segment in image_bv_split:
        encrypted_bv = encrypt(iv, round_keys) #Encrypting initialization vector.
        encrypted_bv ^= image_bv_segment #Xor with image segment
        encrypted_bv.write_to_file(file_out=encrypted_fp) #Write to image file.
        iv = BitVector(size=128, intVal=(iv.int_val()+1)) #Incrementing initialization vector.
